...
const MarqueeUpdates = ({ data }) => {
  ...
  const handleEditMarquee = async () => {
    ...
    const obj = formatPost({ code: "1", key: "marquee", db: JSON.stringify(newData) });
  ...
  return (
    <Container fluid className="main-content-container px-4">
      <Row noGutters className="page-header py-4 space-between">
        <PageTitle
          sm="4"
          title="Marquee Updates"
          subtitle="Marquee"
          className="text-sm-left"
        />
        <Button onClick={handleAddToggle}>Add Marquee Item</Button>
      </Row>
      {alert.show &&
